...
from ..models.entities import (
    BackendApi, GatewayRoute, FrontendUrl, 
    MappingRule, File, Repo
)
...
class Neo4jWriter:
        ...
        with self.driver.session(database=self.config.database) as session:
            for label, keys in constraints:
                ...
                try:
                    ...
                    FOR (n:{label})
                    REQUIRE ({', '.join([f'n.{k}' for k in keys])}) IS NODE KEY
                    """
                    session.run(query)
    ...
    def write_calls_relation(self, url: FrontendUrl, route: GatewayRoute,
    ...
    def write_routes_to_relation(self, route: GatewayRoute, api: BackendApi,
                                  match_type: str, confidence: float) -> None:
        query = """
        MATCH (route:GatewayRoute {
            method: $route_method,
            full_path: $route_full_path,
